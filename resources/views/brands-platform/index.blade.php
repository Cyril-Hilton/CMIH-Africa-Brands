...
.liquid-grid{display:grid!important;grid-template-columns:repeat(5,minmax(0,1fr))!important;gap:16px!important;align-items:start}
.liquid-tile{
  position:relative;min-width:0;min-height:190px;border-radius:26px;padding:17px;overflow:hidden;
...
@media(max-width:620px){
  ...
  .liquid-grid,.home-stats,.publication-grid{grid-template-columns:1fr!important}
...
@section('content')
@php
    $logo = asset('images/logo/logo-light.png');
    ...
    <div id="brands" class="brand-zone">
        ...
        <div class="brand-carousel-wrap">
            <div class="brand-carousel-viewport">
                <div class="liquid-grid">
                    @forelse($brands as $brand)
                        @php
                            $arc = $loop->index % 5;
                            $primary = $brand->primary_color ?: '#ff1020';
                            $secondary = $brand->secondary_color ?: '#9d000d';
                            $accent = $brand->accent_color ?: $primary;
                            $brandKey = $brand->slug ?: $brand->id;
                            $logoUrl = $brand->logoUrl('dark') ?: $brand->logoUrl();
                        @endphp
                        <a href="{{ route('brands-platform.show', $brandKey) }}" class="liquid-tile arc-{{ $arc }}" style="--tile-bg:linear-gradient(145deg, {{ $secondary }}, {{ $primary }}, {{ $accent }});">
                            <div class="tile-logo-wrap">
                                @if($logoUrl)
                                    <img class="tile-logo" src="{{ $logoUrl }}" alt="{{ $brand->name }} logo" loading="lazy">
                                @else
                                    <span class="tile-logo" style="display:grid;place-items:center;width:92px;height:92px;border-radius:26px;background:rgba(255,255,255,.12);font-weight:950;font-size:28px;">
                                        {{ \Illuminate\Support\Str::of($brand->name)->substr(0, 2)->upper() }}
                                    </span>
                                @endif
                            </div>
            ...
            <div class="carousel-controls">
                <a class="carousel-arrow" href="#brands" aria-label="Previous brand">&#8249;</a>
                <div class="carousel-count">{{ $brands->count() }} brands</div>
                <a class="carousel-arrow" href="#brands" aria-label="Next brand">&#8250;</a>
            </div>
        </div>
    </div>
        ...
        <div class="publication-strip">
            ...
            <div class="publication-grid">
                @foreach($recentPublications->take(3) as $publication)
                    <a href="{{ route('brands-platform.show', $publication->brand?->slug ?: $publication->brand_id) }}" class="publication-card">
