<section x-show="activeTab === 'profile'"
         ...
         @if(request('tab', 'home') !== 'profile') style="display: none" @endif
         class="merch-profile-view space-y-6"
         >
    <div class="max-w-4xl mx-auto space-y-6">

        <!-- Staff Profile Header Card -->
        <div class="rounded-2xl p-5 sm:p-6 border border-indigo-100 bg-white shadow-sm flex flex-col sm:flex-row items-center gap-5 sm:gap-6 relative">
            <div class="relative group shrink-0 mx-auto sm:mx-0">
                <div class="relative mx-auto h-28 w-28 shrink-0 overflow-hidden rounded-full bg-white ring-4 ring-indigo-100 shadow-xl">
                ...
                <button type="button" @click="showPhotoModal = true"
                        class="absolute bottom-0 right-0 h-8 w-8 rounded-full bg-[#0F0E9A] text-white flex items-center justify-center text-xs shadow-lg hover:scale-110 transition cursor-pointer z-10 ring-2 ring-white"
                        title="Upload / Change Avatar"
                        style="background-color: #0F0E9A !important; color: #ffffff !important;">
                    📷
                </button>
            </div>
            ...
            <div class="min-w-0 flex-1 text-center sm:text-left space-y-2 w-full">
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                    <div>
                        <h2 class="text-2xl sm:text-3xl font-black text-[#333333] leading-tight">{{ auth()->user()->name }}</h2>
                        <p class="text-xs text-[#0F0E9A] font-extrabold uppercase tracking-wider mt-0.5">{{ auth()->user()->job_title ?: 'Merchandiser Field Agent' }}</p>
                    </div>
                ...
                <!-- Clean Contact Info -->
                <div class="flex flex-col sm:flex-row items-center justify-center sm:justify-start gap-1 sm:gap-3 text-xs text-[#4D4D4D] font-bold pt-1">
                ...
                <div class="pt-3 flex flex-col sm:flex-row items-stretch sm:items-center gap-2 justify-center sm:justify-start">
                    ...
                    <button type="button" onclick="window.location.reload()"
                            ...
                            style="background-color: #E0F2FE !important; color: #0F0E9A !important; border: 1.5px solid #7DD3FC !important;">
                        Refresh / Sync Data
                    </button>
                </div>
            </div>
        ...
        <div class="grid grid-cols-1 sm:flex sm:items-center gap-2 border-b border-indigo-100 pb-3">
            <button type="button" @click="profileSubTab = 'personal'"
                    :style="profileSubTab === 'personal' ? 'background-color: #0F0E9A !important; color: #ffffff !important;' : 'background-color: #E0F2FE; color: #0F0E9A;'"
            ...
            <button type="button" @click="profileSubTab = 'banking'"
                    :style="profileSubTab === 'banking' ? 'background-color: #0F0E9A !important; color: #ffffff !important;' : 'background-color: #E0F2FE; color: #0F0E9A;'"
            ...
            <button type="button" @click="profileSubTab = 'organization'"
                    :style="profileSubTab === 'organization' ? 'background-color: #0F0E9A !important; color: #ffffff !important;' : 'background-color: #E0F2FE; color: #0F0E9A;'"
        ...
        <div x-show="profileSubTab === 'personal'"
             @if(request('subtab', 'personal') !== 'personal') style="display: none" @endif
             class="space-y-6">
            <div class="rounded-2xl p-6 border border-sky-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm space-y-6">
                ...
                <form method="POST" action="{{ route('merchandisers.profile.update') }}" class="space-y-5">
                    ...
                    <div class="border-t border-sky-100 dark:border-slate-800 pt-4 space-y-4">
                        <h4 class="text-xs font-extrabold uppercase tracking-wider text-[#0F0E9A]">Password Reset &amp; Security</h4>
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
        ...
        <div x-show="profileSubTab === 'banking'"
             @if(request('subtab', 'personal') !== 'banking') style="display: none" @endif
             class="space-y-6">
            <div class="rounded-2xl p-6 border border-sky-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm space-y-6">
                <div class="flex items-center justify-between border-b border-sky-100 dark:border-slate-800 pb-4">
                    <div>
                ...
                <form method="POST" action="{{ route('merchandisers.profile.update') }}" class="space-y-5">
                    ...
                    <div class="space-y-4">
                        <h4 class="text-xs font-extrabold uppercase tracking-wider text-[#0F0E9A]">Mobile Money Account</h4>
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
        ...
        <div x-show="profileSubTab === 'organization'"
             @if(request('subtab', 'personal') !== 'organization') style="display: none" @endif
             class="space-y-6">
            <div class="rounded-2xl p-6 border border-sky-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm space-y-5">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    <div class="p-4 rounded-xl border border-sky-100 bg-[#F0F9FF] dark:bg-slate-800">
                        ...
                        <p class="text-sm font-black text-slate-900 dark:text-white mt-1">{{ auth()->user()->merchandiserKd?->name ?? 'Unassigned' }}</p>
                    </div>

                    <div class="p-4 rounded-xl border border-sky-100 bg-[#F0F9FF] dark:bg-slate-800">
                        <p class="text-[10px] font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">Field Region</p>
                        <p class="text-sm font-black text-slate-900 dark:text-white mt-1">{{ auth()->user()->merchandiserRegion?->name ?? 'Unassigned' }}</p>
                    </div>

                    <div class="p-4 rounded-xl border border-sky-100 bg-[#F0F9FF] dark:bg-slate-800">
                        <p class="text-[10px] font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">Direct Supervisor</p>
                        <p class="text-sm font-black text-slate-900 dark:text-white mt-1">{{ auth()->user()->lineManager?->name ?? 'Unassigned' }}</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="rounded-2xl bg-white dark:bg-slate-900 border border-sky-200 dark:border-slate-800 w-full max-w-md p-6 space-y-4 shadow-2xl relative" @click.away="showPhotoModal = false">
            ...
            <form method="POST" action="{{ route('merchandisers.profile.photo.update') }}" enctype="multipart/form-data" class="space-y-4">
                ...
                <div class="flex flex-col items-center gap-3">
                    ...
                    <label class="cursor-pointer px-4 py-2 bg-[#E0F2FE] hover:bg-sky-200 rounded-xl text-xs font-bold text-[#0F0E9A] border border-sky-200 transition">
                        Select Photo File
                ...
                <div class="pt-2 flex justify-end gap-2 border-t border-slate-200 dark:border-slate-800">
                    ...
                    <button type="submit" class="px-5 py-2 rounded-xl bg-[#0F0E9A] text-white text-xs font-bold uppercase tracking-wider hover:bg-blue-700 transition shadow-md">Save Avatar</button>
                </div>
            </form>
        </div>
    </div>
