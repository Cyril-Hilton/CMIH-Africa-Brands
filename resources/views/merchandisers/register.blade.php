<x-guest-layout>
    <div class="space-y-5 sm:space-y-6 min-w-0 max-w-full">
        ...
        <div class="space-y-1.5 sm:space-y-2">
            <p class="text-[10px] sm:text-xs uppercase tracking-[0.2em] sm:tracking-[0.3em] text-brand-ash font-bold">Merchandiser Portal Access</p>
        ...
        <form method="POST" action="{{ route('merchandisers.register') }}" enctype="multipart/form-data" class="space-y-4 min-w-0 max-w-full">
            @csrf

            <!-- Unified Role Selector (Field Agent, Supervisor, Client / TM) -->
            <fieldset class="space-y-2 min-w-0 max-w-full">
                ...
                <div class="grid grid-cols-1 gap-2 sm:grid-cols-3 sm:gap-2.5 min-w-0">
            ...
            <div class="min-w-0">
                ...
                <x-text-input id="name" type="text" name="name" :value="old('name')" required placeholder="Enter your full name" class="w-full min-w-0 text-xs sm:text-sm" />
            ...
            <div class="min-w-0">
                ...
                <div class="mt-1.5 flex items-center gap-3 rounded-xl border border-brand-white/15 bg-brand-white/5 p-2.5 sm:p-3 transition hover:border-brand-red/40 overflow-hidden min-w-0">
                    <input id="profile_photo" type="file" name="profile_photo" accept="image/*" required class="w-full min-w-0 text-xs text-brand-white/80 file:mr-2 file:rounded-lg file:border-0 file:bg-brand-red file:px-2.5 file:py-1 file:text-xs file:font-bold file:text-white hover:file:bg-brand-red/80 cursor-pointer">
                </div>
            ...
            <!-- Login Email Address (Dedicated Row) -->
            <div class="min-w-0">
                <x-input-label for="email" :value="__('Login Email Address')" class="text-xs font-semibold" />
                <x-text-input id="email" type="email" name="email" :value="old('email')" required placeholder="name@company.com" class="w-full min-w-0 text-xs sm:text-sm" />
                <x-input-error :messages="$errors->get('email')" class="mt-1" />
            </div>

            <!-- Personal/Contact Email (Dedicated Row) -->
            <div class="min-w-0">
                <x-input-label for="contact_email" :value="__('Personal/Contact Email')" class="text-xs font-semibold" />
                <x-text-input id="contact_email" type="email" name="contact_email" :value="old('contact_email')" required placeholder="personal@email.com" class="w-full min-w-0 text-xs sm:text-sm" />
                <x-input-error :messages="$errors->get('contact_email')" class="mt-1" />
            </div>

            <!-- Phone Number (Dedicated Row) -->
            <div class="min-w-0">
                <x-input-label for="phone" :value="__('Phone Number')" class="text-xs font-semibold" />
                <x-text-input id="phone" type="text" name="phone" :value="old('phone')" required placeholder="e.g. +23354XXXXXXX" class="w-full min-w-0 text-xs sm:text-sm" />
                <x-input-error :messages="$errors->get('phone')" class="mt-1" />
            </div>

            <!-- Date of Birth (Dedicated Row) -->
            <div class="min-w-0">
                <x-input-label for="date_of_birth" :value="__('Date of Birth')" class="text-xs font-semibold" />
                <x-text-input id="date_of_birth" type="date" name="date_of_birth" :value="old('date_of_birth')" required class="w-full min-w-0 text-xs sm:text-sm" />
            ...
            <fieldset class="space-y-2 min-w-0 max-w-full">
                ...
                <div class="grid grid-cols-1 gap-2.5 sm:grid-cols-2 min-w-0">
                    @foreach($merchandiserTenants as $tenant)
                        <label class="group cursor-pointer rounded-xl border border-brand-white/10 bg-brand-white/5 p-3 sm:p-3.5 transition hover:border-brand-red/45 hover:bg-brand-red/5 active:scale-[0.99] min-w-0">
            ...
            <!-- Password (Dedicated Row) -->
            <div class="min-w-0">
                <x-input-label for="password" :value="__('Password')" class="text-xs font-semibold" />
                <x-text-input id="password" type="password" name="password" required autocomplete="new-password" placeholder="At least 9 characters" class="w-full min-w-0 text-xs sm:text-sm" />
                <x-input-error :messages="$errors->get('password')" class="mt-1" />
            </div>

            <!-- Confirm Password (Dedicated Row) -->
            <div class="min-w-0">
                <x-input-label for="password_confirmation" :value="__('Confirm Password')" class="text-xs font-semibold" />
                <x-text-input id="password_confirmation" type="password" name="password_confirmation" required placeholder="Repeat password" class="w-full min-w-0 text-xs sm:text-sm" />
