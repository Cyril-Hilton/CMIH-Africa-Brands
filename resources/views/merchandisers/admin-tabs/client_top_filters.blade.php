...
<div class="merch-card rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-4 shadow-sm mb-6">
    <form method="GET" action="{{ route('merchandisers.client.dashboard') }}" class="flex flex-wrap items-center justify-between gap-3">
        ...
        <div class="flex items-center gap-1 bg-slate-100 dark:bg-slate-800 p-1 rounded-xl">
            ...
            <button type="submit" name="perf_period" value="week" class="px-3 py-1.5 rounded-lg text-xs font-bold transition-all {{ $activePeriod === 'week' ? 'bg-white dark:bg-slate-900 text-slate-900 dark:text-white shadow-xs font-black' : 'text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white' }}">
                <i class="fa-solid fa-calendar-week mr-1"></i> Week
            </button>
            <button type="submit" name="perf_period" value="month" class="px-3 py-1.5 rounded-lg text-xs font-bold transition-all {{ $activePeriod === 'month' ? 'bg-white dark:bg-slate-900 text-slate-900 dark:text-white shadow-xs font-black' : 'text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white' }}">
        ...
        <div class="flex flex-wrap items-center gap-2.5 min-w-0">
            ...
            <div class="flex items-center gap-1.5 min-w-0">
                ...
                <select name="performance_region_id" onchange="this.form.submit()" class="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-950 px-3 py-1.5 text-xs text-slate-900 dark:text-white font-bold focus:ring-0">
            ...
            <div class="flex items-center gap-1.5 min-w-0">
                ...
                <select name="performance_kd_id" onchange="this.form.submit()" class="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-950 px-3 py-1.5 text-xs text-slate-900 dark:text-white font-bold focus:ring-0">
            ...
            @if($showStore)
                <div class="flex items-center gap-1.5 min-w-0">
                    ...
                    <select name="performance_outlet_id" onchange="this.form.submit()" class="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-950 px-3 py-1.5 text-xs text-slate-900 dark:text-white font-bold focus:ring-0">
            ...
            @if(filled($filters['region_id'] ?? null) || filled($filters['kd_id'] ?? null) || filled($filters['outlet_id'] ?? null))
                <a href="{{ route('merchandisers.client.dashboard', ['view' => $currentView, 'tenant' => $merchTenant['code'] ?? 'unilever']) }}" class="px-2.5 py-1.5 rounded-xl bg-slate-100 dark:bg-slate-800 text-[11px] font-bold text-slate-600 dark:text-slate-300 hover:text-rose-500 transition">
