...
<div class="merch-card rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-4 shadow-sm mb-6">
    <form method="GET" action="{{ route('merchandisers.client.dashboard') }}" onsubmit="if(window.showClientPortalLoader) window.showClientPortalLoader('Applying filters...')" class="flex flex-wrap items-center justify-between gap-3">
        ...
        <div class="flex items-center gap-1 bg-slate-100 dark:bg-slate-800 p-1 rounded-xl">
            <button type="submit" name="perf_period" value="day" onclick="if(window.showClientPortalLoader) window.showClientPortalLoader('Loading daily period...')" class="px-3 py-1.5 rounded-lg text-xs font-bold transition-all {{ $activePeriod === 'day' ? 'bg-white dark:bg-slate-900 text-slate-900 dark:text-white shadow-xs font-black' : 'text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white' }}">
                <i class="fa-solid fa-calendar-day mr-1"></i> Day
            </button>
            <button type="submit" name="perf_period" value="week" onclick="if(window.showClientPortalLoader) window.showClientPortalLoader('Loading weekly period...')" class="px-3 py-1.5 rounded-lg text-xs font-bold transition-all {{ $activePeriod === 'week' ? 'bg-white dark:bg-slate-900 text-slate-900 dark:text-white shadow-xs font-black' : 'text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white' }}">
                <i class="fa-solid fa-calendar-week mr-1"></i> Week
            </button>
            <button type="submit" name="perf_period" value="month" onclick="if(window.showClientPortalLoader) window.showClientPortalLoader('Loading monthly period...')" class="px-3 py-1.5 rounded-lg text-xs font-bold transition-all {{ $activePeriod === 'month' ? 'bg-white dark:bg-slate-900 text-slate-900 dark:text-white shadow-xs font-black' : 'text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white' }}">
        ...
        <div class="flex flex-wrap items-center gap-2.5 min-w-0">
            ...
            @if($showDateRange)
                <div class="flex items-center gap-1.5">
                    ...
                    <input type="date" name="clock_from" value="{{ request('clock_from') ?? ($clockFromInput ?? '') }}" onchange="if(window.submitFilterForm) window.submitFilterForm(this, 'Filtering date range...'); else this.form.submit()" class="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-950 px-2.5 py-1.5 text-xs text-slate-900 dark:text-white font-bold focus:ring-0">
                </div>
                <div class="flex items-center gap-1.5">
                    <span class="text-[10px] uppercase font-extrabold text-slate-500 dark:text-slate-400 tracking-wider">To:</span>
                    <input type="date" name="clock_to" value="{{ request('clock_to') ?? ($clockToInput ?? '') }}" onchange="if(window.submitFilterForm) window.submitFilterForm(this, 'Filtering date range...'); else this.form.submit()" class="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-950 px-2.5 py-1.5 text-xs text-slate-900 dark:text-white font-bold focus:ring-0">
                </div>
            @endif
